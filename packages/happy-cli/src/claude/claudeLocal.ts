...
export async function claudeLocal(opts: {
    ...
    // Use HTTP for thinking state since PTY doesn't support extra fd
    if (opts.thinkingUrl) {
        env.HAPPY_THINKING_URL = opts.thinkingUrl;
    }

    logger.debug(`[ClaudeLocal] Spawning via PTY: ${claudeCliPath}`);
    logger.debug(`[ClaudeLocal] Args: ${JSON.stringify(args)}`);

    // TODO: sandbox support with PTY
    ...
    // Forward PTY output → local terminal
    ptyProcess.onData((data: string) => {
        process.stdout.write(data);
    });

    // Forward local stdin → PTY
